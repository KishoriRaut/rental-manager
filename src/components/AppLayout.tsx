...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-sidebar text-sidebar-foreground transform transition-transform duration-200 lg:relative lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-14 items-center gap-4 border-b bg-card px-4 lg:px-6">
          ...
          <h1 className="text-lg font-display font-semibold">
            {navItems.find(i => i.path === "/" ? location.pathname === "/" : location.pathname.startsWith(i.path))?.label || ""}
          </h1>
        </header>
