-- Fix auth.users reference issue by removing foreign key constraint to auth.users
-- The auth.users table is managed by Supabase and may not be available during migration

-- Drop existing tables
DROP TABLE IF EXISTS public.monthly_billing CASCADE;
DROP TABLE IF EXISTS public.tenants CASCADE;
DROP TABLE IF EXISTS public.houses CASCADE;

-- Recreate houses table without auth.users foreign key (RLS will handle security)
...
-- Recreate monthly billing table
...
-- Create RLS policies
CREATE POLICY "Users can manage own houses" ON public.houses FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can manage own tenants" ON public.tenants FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can manage own billing" ON public.monthly_billing FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
